...
void inandout(Student *head) //学生进出小审批子模块
{
    int r;
...
void searchforper(Student *cnt) //查询学生信息
{
    cout << "学院：" << cnt->information.school << endl;
    cout << "姓名：" << cnt->information.name << endl;
...
void applyforinandout(Student *head) //学生进出校申请
{
    int n;
    bool flag = false;
    while (true)
...
void del(Student *head) //删除申请
{
    head->information.Apply.state = false;
}

void change(Student *head) //更改申请
{
    int r;
    bool flag2 = false;
    cout << "修改校区请按 1" << endl;
